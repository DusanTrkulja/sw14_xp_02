...
public class OtrChatManager implements OtrEngineListener, OtrSmEngineHost {
    ...
    private static String LOG_TAG = "OtrChatManager";
    ...
    private OtrChatManager(int otrPolicy, OtrKeyManager otrKeyManager) throws Exception {
        //TODO Perhaps imService imRemoteService is not used
        mOtrEngineHost = new OtrEngineHostImpl(new OtrPolicyImpl(otrPolicy), otrKeyManager);
...
//        mContext = imService;
    ...
    public static synchronized OtrChatManager getInstance(int otrPolicy, OtrKeyManager otrKeyManager)
            throws Exception {
        if (mInstance == null) {
            mInstance = new OtrChatManager(otrPolicy, otrKeyManager);
        }
    ...
    public void transformSending(String message, String to) {
        transformSending(message, to, false, null);
    }
    
    public void transformSending(String message, String from, boolean isResponse, byte[] data) {
        String localUserId = Common.getPreferredEmail();

        SessionID sessionId = getSessionId(localUserId, from);
        ...
        if (mOtrEngine != null && sessionId != null) {
            ...
            if (data != null && sessionStatus != SessionStatus.ENCRYPTED) {
                ...
                startSession(localUserId, from);
                Log.d(LOG_TAG, "auto-start OTR on data send request");
                return;
            }
            ...
            try {
                ...
                if (sessionStatus != SessionStatus.PLAINTEXT || sessionPolicy.getRequireEncryption()) {
                    message = mOtrEngine.transformSending(sessionId, message, isResponse, data);
                ...
                Log.e(LOG_TAG, "error encrypting");
            }
        }
    }
    ...
    public void sessionStatusChanged(SessionID sessionID) {
        ...
        Log.d(LOG_TAG, "session status changed: " + sStatus);
    ...
    public String getLocalKeyFingerprint(String localUser, String remoteUser) {
        return mOtrEngineHost.getLocalKeyFingerprint(getSessionId(localUser, remoteUser));
    }
    ...
    //Combine with ChatActivity ask for secret if not encrypted
    @Override
    public void askForSecret(SessionID sessionID, String question) {
        ...
        if (connection == null) {
            Log.d(LOG_TAG, "Could ask for secret - no connection for " + sessionID.getSessionId());
            return;
        }
